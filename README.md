...
**스프링 부트와 React를 활용한 실시간 헬스장 인원 관리 및 JWT 기반 인증 서비스입니다.**
...
- **Frontend**: React (Vite), Axios, jwt-decode
...
- **실시간 혼잡도 계산**: 현재 인원과 최대 수용 정원을 비교하여 3단계 상태를 시각화합니다.
  - $$Congestion Rate (\%) = \left( \frac{\text{Current Count}}{\text{Max Capacity}} \right) \times 100$$
  - **여유**: 30% 이하 (초록)
  - **보통**: 31% ~ 70% (주황)
  - **혼잡**: 71% 이상 (빨강)
- **입/퇴장 시스템**: 유저별 입퇴장 기록(`VisitLog`) 관리 및 실시간 인원 반영.
- **동적 유저 식별**: 클라이언트에서 JWT 토큰을 해독(`jwt-decode`)하여 로그인한 유저의 고유 ID를 기반으로 체크인을 처리합니다.

### 2. 보안 및 사용자 인증

- **BCrypt 암호화**: 사용자 비밀번호를 단방향 해시 함수로 암호화하여 DB 보안 강화.
- **JWT 기반 Stateless 인증**: 서버가 상태를 저장하지 않고 토큰 검증만으로 권한을 확인하여 서버 확장성 확보.
...
- **전역 예외 처리**: `GlobalExceptionHandler`를 통해 500 에러 대신 비즈니스 의미가 담긴 커스텀 에러 메시지 응답 구현.
...
- **해결**: `Keys.hmacShaKeyFor()`와 `StandardCharsets.UTF_8`을 활용하여 문자열을 바이트 배열로 직접 전달하는 표준 방식으로 수정.
